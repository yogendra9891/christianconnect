...
$user		= JFactory::getUser();
	        ...
	        $link = JRoute::_( 'index.php?option=com_christianconnect&task=church.edit&id='. (int)$row->id );
	...
	<tr class="row<?php echo $i % 2; ?>">
		...
		<td>
		 <?php if (isset($row->cname)) { ?>
			<?php if ($canEdit) : ?>
			<a href="<?php echo $link; ?>"><?php echo $row->cname; ?></a>
			<?php else : ?>
						<?php echo $row->cname; ?>
					<?php endif; ?>
		<?php } ?> 
		...
		<td>
		 ...
		 <?php echo JHtml::_('jgrid.published', $row->published, $i, 'charges.', $canChange, 'cb'); ?>
