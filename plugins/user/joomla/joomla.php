...
class plgUserJoomla extends JPlugin
{
	...
	public function onUserLogout($user, $options = array())
	{
		...
		//add by yogendra...for deleting the current sessionid...here we are checking if the session id is set from webservice profile logout call.
		...
		$db = JFactory::getDBO();
		//last AND clause added by yogendra..because we are deleting the session according to the sessionid..
		$db->setQuery(
			'DELETE FROM '.$db->quoteName('#__session') .
			' WHERE '.$db->quoteName('userid').' = '.(int) $user['id'] .
			' AND '.$db->quoteName('client_id').' = '.(int) $options['clientid'].
			' AND '.$db->quoteName('session_id').' = '.$sessionid
		); 
		$db->query();
