    ...
    public function Logout($data){
	 ...
	 /*function to response data
	  ...
	  private function responseData($errNo,$msgString,$result){
		 ...
		 {
		 	$userProfileObject=new stdClass();
		 	...
		 	$query->select('a.userid as userId, a.fname As firstName, a.lname AS lastName, a.email,DATE_FORMAT(a.dob, "%Y-%m-%d") AS DOB, a.city AS town, b.name AS religion,a.interest,a.favouritebiblequote AS favoriteBibleQuotes, a.profileimage AS profilePic');
		 	$query->from('#__christianusers  AS a');
		 	$query->join('LEFT','#__userreligion AS b ON b.id = a.religion AND b.published=1');
                        $query->join('LEFT','#__users AS c ON c.id = a.userid');
		 	$query->where('a.userid='.$userId);
                        $query->where('c.block=0');
		 	
		 	$db->setQuery($query);
		 	$userProfileObject=$db->loadObject();
		 	if(count($userProfileObject)!==0)
		 	{
		 		$userProfileObject->localChurch=new stdClass();
		 		$userProfileObject->otherChurch=new stdClass();
		 	
		 	$userProfileObject->localChurch=$this->getLocalChurchByUserId();
		 	
		 	$userProfileObject->otherChurch=$this->getOtherChurchByUserId();
		 	}
		 ...
		 function getLocalChurchByUserId()
		 {
			...
			$subquery->select('b.localchurch AS localChurch');
		...
		/*function to get church by userid
		 ...
		 function getOtherChurchByUserId()
		 {
			$userId=$this->getState('profile.userId');
		 	...
		 	$subquery->clear();
		 	//To Do keep this query in subquery
			$subquery->select('b.otherchurch as otherChurch');
		 	$subquery->from('#__userchurch  AS b');
		 	$subquery->where('b.userid='.$userId);
		 	$db->setQuery($subquery);
