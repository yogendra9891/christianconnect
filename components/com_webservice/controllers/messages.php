...
class WebserviceControllerMessages extends WebserviceController
{
	...
	function getUnreadMessageCount()
	{
	 ...
	 public function deleteMessage(){
		 ...
		 if(!empty($data))
		 {
		    ...
		    if($return)
		    {
		    	echo $this->responseData(JText::_('ERRCODE_SUCCESS'),  JText::_('MSG_DELETEMESSAGES_SUCCESS'),'');
				exit;
		    }
		    else{
		    	echo $this->responseData(JText::_('ERRCODE_DELETE_MESSAGE_FAILURE'),  JText::_('MSG_DELETEMESSAGES_FAILURE'),'');
				exit;
		    }
		 }
	 }
 ...
 /*function to save reply Message
	 * @param session id
	 * @return successfull message if successfull
	 */
	function replyMessage()
	{
		 ...
		 if(!empty($data))
		 {
			...
			$model=$this->getModel();
			$model->setState('messages.userId',$userId);
			$model->setState('messages.messageObject',$data['messageObject']);
		    $return=$model->replyMessage();
		    if($return)
		    {
		    	echo $this->responseData(JText::_('ERRCODE_SUCCESS'),  JText::_('MSG_REPLYMESSAGE_SUCCESS'),'');
				exit;
		    }else 
		    {
		    	echo $this->responseData(JText::_('ERRCODE_REPLYMESSAGE_FAIL'),  JText::_('MSG_REPLYMESSAGE_FAIL'),'');
				exit;
		    }
...
/*function to get message Thread
	...
	function getMessageThreads()
	{
		 ...
		 if(!empty($data))
		 {
			...
			$model=$this->getModel();
			$model->setState('messages.userId',$userId);
			$model->setState('messages.data',$data);
			$return=$model->getMessageThreads();
		    if($return)
		    {
		    	echo $this->responseData(JText::_('ERRCODE_SUCCESS'),  JText::_('MSG_GETMESSAGETHREAD_SUCCESS'),$return);
				exit;
		    }
